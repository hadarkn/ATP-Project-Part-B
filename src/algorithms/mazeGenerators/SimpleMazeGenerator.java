...
public class SimpleMazeGenerator extends AMazeGenerator {
    ...
    /**
     * Generates a simple maze with random walls and empty spaces.
     * Ensures that start and goal positions are solvable and not the same.
     *
    ...
    public Maze generate(int rows, int cols) {
        ...
        // Fill the maze with random walls and empty spaces
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                maze.setCell(i, j, rand.nextInt(2)); // Randomly 0 (empty) or 1 (wall)
            }
        }
